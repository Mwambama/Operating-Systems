...
// --- Fix for implicit declaration warnings (strdup) ---
// Explicitly declare strdup prototype as it's often missing in standard C libraries
...
// ADDED: Single mutex to protect the entire bank database
...
struct trans { int acc_id; int amount; };
struct request { 
    struct request *next; int request_id; char request_type; 
    int check_acc_id; struct trans *transactions; int num_trans;
    struct timeval starttime, endtime; 
};
struct queue { 
    struct request *head, *tail; int next_request_id;
    int num_jobs; int end_flag; 
} request_queue;
...
// --- Worker Processing Logic (COARSE-GRAINED IMPLEMENTATION) ---

void process_check(struct request *req) {
    int id = req->check_acc_id;
    
    // COARSE LOCK: Lock the entire bank for one account read
...
void process_transaction(struct request *req) {
    // Collect involved IDs (needed for reads/writes)
    int *all_ids = (int *)malloc(req->num_trans * sizeof(int));
    for (int i = 0; i < req->num_trans; i++) {
        all_ids[i] = req->transactions[i].acc_id;
    }
    
    // 1. Acquire the single Coarse Lock
    pthread_mutex_lock(&bank_lock);
    
    // 2. Atomicity Check (Read & Verify Balances)
    ...
    if (insufficient_acc_id == -1) {
        ...
        // ISF: Output failure, state remains original
        gettimeofday(&req->endtime, NULL);
        pthread_mutex_lock(&output_mutex);
    ...
    // 4. Release the single Coarse Lock
    pthread_mutex_unlock(&bank_lock);
...
void *worker_thread(void *arg) {
    // Correctly handle the unused 'arg' parameter
    (void)arg; 
    struct request *req;
...
// --- Main Function (Initialization Changes) ---
int main(int argc, char **argv) {
    if (argc != 4) {
        fprintf(stderr, "Usage: ./appserver-coarse <# of worker threads> <# of accounts> <output file>\n");
        return 1;
    }
    ...
    // CRITICAL COARSE-GRAINED INITIALIZATION: Initialize the single bank lock.
    pthread_mutex_init(&bank_lock, NULL); 
    
    // NOTE: The fine-grained account_locks array and loop are intentionally omitted here.
