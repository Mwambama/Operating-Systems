...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int oldest_arrival_time = INT_MAX; 

	/* FIFO: Evict the page that has been in memory the longest (earliest time_of_arrival) */
	for (int i = 0; i < num_frames; i++)
	{
		if (PageFrames[i].time_of_arrival < oldest_arrival_time)
		{
			oldest_arrival_time = PageFrames[i].time_of_arrival;
			frame_to_evict = i;
		}
	}
...
{
	int frame_to_evict = 0;
	int earliest_access_time = INT_MAX;

	/* LRU: Evict the page whose most recent access time is the earliest (time_of_access) */
	for (int i = 0; i < num_frames; i++)
	{
		if (PageFrames[i].time_of_access < earliest_access_time)
		{
			earliest_access_time = PageFrames[i].time_of_access;
			frame_to_evict = i;
		}
	}
...
{
	int frame_to_evict = 0;
	// Max future access time tracks the time index of the farthest page access
	int max_future_access_time = -1; 
	
	/* OPT: Evict the page whose next use will occur farthest in the future. */
	for (int i = 0; i < num_frames; i++)
	{
		int page_id_in_frame = PageFrames[i].page_id;
		// Default: Page is never accessed again (sets maximum distance)
		int next_access_time = num_accesses; 
		
		/* Search the future access sequence (from the next instruction onward) */
		for (int j = current_access + 1; j < num_accesses; j++)
		{
			// Check if the page in frame i is accessed at time j
			if (PageAccesses[j] == page_id_in_frame)
			{
				next_access_time = j; // Found its next use
				break; 
			}
		}

		/* Compare the future access time (distance) with the max found so far */
		if (next_access_time > max_future_access_time)
		{
			max_future_access_time = next_access_time;
			frame_to_evict = i;
		}
	}
